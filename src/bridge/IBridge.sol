...
interface IBridge {
    ...
    ///      stateful-precompile failure burns all gas forwarded to the token call). Probe
    ///      deliverability with an `eth_call deliver(srcCID, nonce)` and size batches accordingly.
